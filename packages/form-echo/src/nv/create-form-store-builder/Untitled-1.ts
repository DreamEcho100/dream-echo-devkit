...
export type ValidInputResolverSchema<Values, ErrorShape> = {
...
>;
type ResolverHandlerParam<
  Values,
  Resolvers extends ResolverBaseShape,
  Path extends string,
> = {
...
interface FormStoreShape<Values, InputResolverSchema, ResolverError = Error> {
  fields: {
    values: Values;
  ...
  resolvers: {
    ...
    errors: {
      fields: {
        [Key in GetPaths<InputResolverSchema>]?: ResolverError | null;
      };
      count: number;
    };
